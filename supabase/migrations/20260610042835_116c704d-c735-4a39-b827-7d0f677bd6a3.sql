...
BEGIN
  ...
  -- Deduction type is stored as a "[Payroll]" / "[Paid Leave]" prefix in reason.
  -- "Payroll" leaves are paid through payroll, NOT from the leave balance.
  ...
  -- If the user chose "Payroll" as the deduction type, do NOT touch the
  -- Annual Leave balance (it is compensated via payroll instead). Leave in
  -- Lieu has no payment option, so it is never affected by this rule.
