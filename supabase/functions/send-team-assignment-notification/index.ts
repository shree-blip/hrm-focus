...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const payload: TeamAssignmentPayload = await req.json();
    ...
    const emailSubject = `👥 You've Been Added to ${manager_name}'s Team`;
    const emailHtml = buildTeamAssignmentEmail(
      assigner_name,
      assigner_email,
      employee_name,
      manager_name
    ...
    // Send to the employee who was added
    ...
    for (const email of allEmails) {
      ...
      const { data: logEntry, error: logError } = await supabase
        ...
        .insert({
          ...
          payload: { assigner_name, assigner_email, employee_name, employee_email, manager_name },
          status: "pending",
          attempts: 0,
    ...
    console.error("send-team-assignment-notification error:", error);
    return new Response(
    ...
    if (attempt >= 3) {
      ...
      for (const admin of admins || []) {
        await supabase.rpc("create_notification", {
          ...
          p_message: `Failed to deliver team assignment email to ${to} after 3 attempts. Error: ${errorMsg}`,
          p_type: "warning",
