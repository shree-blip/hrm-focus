    ...
    <div className="space-y-6">
      ...
      <fieldset disabled={disabled} className="space-y-3">
        ...
        <div>
          ...
          <Textarea rows={2} value={formData.sender_address} onChange={e => onChange({ sender_address: e.target.value })} placeholder="Your address" />
        </div>
        <div>
      ...
      {/* INVOICE DETAILS */}
      <fieldset disabled={disabled} className="space-y-3">
        <legend className="text-[10px] uppercase tracking-widest text-muted-foreground font-semibold mb-2">Invoice Details</legend>
        <div>
        ...
        <div>
          ...
          <Input value={formData.invoice_number} onChange={e => onChange({ invoice_number: e.target.value })} placeholder="INV-001" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label className="text-xs">Invoice Date</Label>
            <Input type="date" value={formData.invoice_date} onChange={e => onChange({ invoice_date: e.target.value })} />
          </div>
          <div>
      ...
      <fieldset disabled={disabled} className="space-y-3">
        <legend className="text-[10px] uppercase tracking-widest text-muted-foreground font-semibold mb-2">Payment Instructions</legend>
        <div>
          <Label className="text-xs">Account Name</Label>
          <Input value={formData.payment_account_name} onChange={e => onChange({ payment_account_name: e.target.value })} />
        </div>
        <div>
        ...
        <div>
          ...
          <Input value={formData.payment_account_number} onChange={e => onChange({ payment_account_number: e.target.value })} />
        </div>
        <div>
          <Label className="text-xs">SWIFT / BIC Code</Label>
          <Input value={formData.payment_swift_code} onChange={e => onChange({ payment_swift_code: e.target.value })} />
        </div>
      </fieldset>
    </div>
  );
