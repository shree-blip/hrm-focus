...
import { Calendar, Plus, Check, X, ChevronLeft, ChevronRight, Users } from "lucide-react";
...
const initialLeaveRequests: LeaveRequest[] = [
  { id: 1, name: "Sarah Johnson", initials: "SJ", type: "Annual Leave", startDate: "Dec 27, 2025", endDate: "Dec 29, 2025", days: 3, status: "pending", reason: "Family vacation during the holidays" },
  { id: 2, name: "Michael Chen", initials: "MC", type: "Sick Leave", startDate: "Dec 26, 2025", endDate: "Dec 26, 2025", days: 1, status: "pending", reason: "Not feeling well" },
  { id: 3, name: "Emily Davis", initials: "ED", type: "Personal Leave", startDate: "Jan 2, 2026", endDate: "Jan 3, 2026", days: 2, status: "approved", reason: "Personal errands" },
  { id: 4, name: "James Wilson", initials: "JW", type: "Annual Leave", startDate: "Dec 23, 2025", endDate: "Dec 24, 2025", days: 2, status: "rejected", reason: "Year-end closing period" },
];
...
const Leave = () => {
  ...
  const handleSubmitRequest = (request: { type: string; startDate: Date; endDate: Date; reason: string }) => {
    ...
    const newRequest: LeaveRequest = {
      ...
      initials: "GD",
      type: request.type,
      startDate: format(request.startDate, "MMM d, yyyy"),
      endDate: format(request.endDate, "MMM d, yyyy"),
      days,
      status: "pending",
            ...
            <CardContent className="pt-6">
              ...
              <div className="space-y-2">
                ...
                <div className="h-2 bg-secondary rounded-full overflow-hidden">
                  <div className={cn("h-full rounded-full transition-all duration-500", balance.color)} style={{ width: `${(balance.used / balance.total) * 100}%` }} />
                </div>
        ...
        <Card className="lg:col-span-2 animate-slide-up opacity-0" style={{ animationDelay: "300ms", animationFillMode: "forwards" }}>
          ...
          <CardContent className="space-y-4">
            {filteredRequests.map((request, index) => (
              <div key={request.id} className="flex items-start gap-4 p-4 rounded-xl bg-accent/30 border border-border hover:border-primary/20 transition-all animate-fade-in" style={{ animationDelay: `${400 + index * 100}ms` }}>
                <Avatar className="h-10 w-10">
                  <AvatarImage src="" />
                  <AvatarFallback className="bg-primary/10 text-primary font-medium">{request.initials}</AvatarFallback>
                </Avatar>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="font-medium">{request.name}</p>
                      <p className="text-sm text-muted-foreground mt-0.5">{request.type} • {request.startDate} - {request.endDate}</p>
          ...
          <CardContent>
            <div className="grid grid-cols-7 gap-1 text-center mb-4">
              ...
              {Array.from({ length: 31 }, (_, i) => i + 1).map((day) => (
                <div key={day} className={cn("text-sm py-2 rounded-md cursor-pointer transition-colors", day === 26 && "bg-primary text-primary-foreground font-medium", (day === 27 || day === 28 || day === 29) && "bg-warning/20 text-warning", day === 25 && "bg-muted text-muted-foreground")}>{day}</div>
              ))}
            </div>
            <div className="space-y-2 pt-4 border-t border-border">
              <p className="text-sm font-medium mb-2">Who's Out</p>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-sm"><div className="h-2 w-2 rounded-full bg-warning" /><span>Sarah J. - Dec 27-29</span></div>
                <div className="flex items-center gap-2 text-sm"><div className="h-2 w-2 rounded-full bg-info" /><span>Michael C. - Dec 26</span></div>
              </div>
            </div>
          </CardContent>
        </Card>
